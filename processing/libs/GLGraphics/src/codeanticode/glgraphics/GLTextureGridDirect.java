...
public class GLTextureGridDirect extends GLTextureGrid {
  ...
  public void render(int sW, int sH, int dW, int dH, int l, 
    ...
    for (int j = 0; j < resY - 1; j++) {
      ...
      for (int i = 0; i < resX; i++) {
        ...
        setTexCoord(i, j, l, s0, s1, t0, t1);
        gl.glVertex2f(p0.x, p0.y);

        setTexCoord(i, j + 1, l, s0, s1, t0, t1);
  ...
  protected void setTexCoord(int i, int j, int l, float sc0, float sc1, float tc0, float tc1) {
    GLTexturedPoint p = new GLTexturedPoint();
    float s, t;
    for (int k = 1; k <= l; k++) {
      getPoint(k, i, j, p);
      s = PApplet.map(p.x, 0, 1, sc0, sc1);
      t = PApplet.map(p.y, 0, 1, tc0, tc1);
      gl.glMultiTexCoord2f(GL.GL_TEXTURE0 + k - 1, s, t);
    }
  }
